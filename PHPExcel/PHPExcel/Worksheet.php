...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
   ...
   /**
    ...
    //Added by LHQ
    //根据Ｅｘｃｅｌ列名获得列的位置序号（从０开始）。
    //支持超过２６列
    //A => 0,AA => 26,AZ => 51,BA => 52,ZZ => 701, AAA => 702
    public function GetColumnIndex($colName) {
        $colIndex = 0;
        $len = strlen($colName);
        for ($i = $len; $i > 1; $i--) {
            $colIndex = ($colIndex + (ord(substr($colName, $len - $i, 1)) - 64)) * 26;
        }
        $colIndex += ord(substr($colName, $len - 1, 1)) - 65;
        return $colIndex;
    }
    //Added by LHQ
    //根据Ｅｘｃｅｌ列的位置序号（从０开始）获得列名。
    //支持超过２６列
    //A <= 0,AA <= 26,AZ <= 51,BA <= 52,ZZ <= 701, AAA <= 702
    public function GetColumnName($colIndex)
    {
        $colName = "";
        if ($colIndex < 26) {
            $colName = chr(65 + $colIndex);
        } else {
            $colName = chr(65 + $colIndex % 26);
            $colIndex = floor($colIndex / 26);
            while ($colIndex > 26) {
                $colName = chr(64 + $colIndex % 26) . $colName;
                $colIndex = floor($colIndex / 26);
            }
            $colName = chr(64 + $colIndex) . $colName;
        }
        return $colName;
    }
